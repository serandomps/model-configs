...
dust.loadSource(dust.compile(require('./template'), 'configs-remove'));

module.exports = function (ctx, container, options, done) {
    var sandbox = container.sandbox;
    Page.findOne({id: options.id}, function (err, config) {
        if (err) return done(err);
        dust.render('configs-remove', serand.pack(config, container), function (err, out) {
            if (err) {
                return done(err);
            }
